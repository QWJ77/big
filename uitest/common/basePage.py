...
from selenium.webdriver.support import expected_conditions as EC  # 用于处理异常
...
class BasePage():
    ...
    def findElement(self, locator):
        ...
        if not isinstance(locator, tuple):
            ...
            try:
                ...
                self.get_screen()
                self.logger.error("定位元素{}失败，未找到该元素".format(locator))
    ...
    def is_value_in_element(self, locator, _value=''):
        '''判断元素value是否为XX，回bool值, value为空字符串，返回Fasle'''
    ...
    def get_screen(self,file_name=time.strftime('%Y-%m-%d_%H-%S-%M', time.localtime(time.time()))):
        ...
        try:
            ...
            # dit = os.path.abspath('..')
            dit = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
            file_name=r"{}\screen\{}.png".format(dit, file_name)
